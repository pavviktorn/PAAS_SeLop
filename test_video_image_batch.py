...
"""PAAS_SeLop batch tester over a folder tree of images + videos -- multi-GPU batching.

Mirrors PAAS_ensemble_v2/test_video_image_batch.py (same discovery, file-sharding across GPUs,
real cross-file batching, and the SAME unified line format) but drives a single SeLop/LROR
detector instead of the FFAA+ensemble pipeline:
  ...
  * each worker writes shard-suffixed results; the main process merges them into results_selop.txt
    in the unified format  (OK/XX/SK/ER  truth=..  pred=..  type=..  fake=..  match=..  <path>)
    and prints a per-label summary.

Examples:
  python3.12 test_video_image_batch.py --ckpt runs/selop_default/best.pt \
      --input-dir /datasets/work/vLLM/data/axonlabs_data_1 --out-dir runs/test --devices all
  python3.12 test_video_image_batch.py --ckpt runs/selop_default/best.pt --input-dir /data --device 1
"""
import argparse
import json
...
def gpu_worker(device_id, worker_idx, media_paths, args_dict, n_workers, resume_files, result_q):
    ...
    from selop.data import build_transforms, class_names
    ...
    with open(args.config) as f:
        cfg = json.load(f)
    amp = {"bf16": torch.bfloat16, "fp16": torch.float16, "fp32": torch.float32}[cfg.get("amp_dtype", "bf16")]
    device = torch.device("cuda:0")
    model, ck, mc = load_model(args.ckpt, cfg["clip_path"], device, amp)
    ...
    print(f"[GPU {device_id}] SeLop ready (num_classes={num_classes}) threshold={thr:.4f} "
          f"({len(media_paths)} media files in shard)", flush=True)

    sfx = f".shard{worker_idx}"
    shard_path = os.path.join(args.out_dir, f"results_selop{sfx}.txt")
    ...
    def flush():
        ...
        try:
            ...
            with torch.autocast(device_type="cuda", dtype=amp, enabled=(amp != torch.float32)):
                logits = model(x)
            p_fake = fake_probability(logits, num_classes).float().cpu().numpy()
            pred_cls = logits.float().argmax(dim=-1).cpu().numpy()
...
def main():
    ...
    ap.add_argument("--ckpt", default="runs/selop_default/best_bin_AUC0.9909.pt")
    ...
    ap.add_argument("--devices", default="2,3", help="CUDA devices: 'all' or e.g. '0,1,2,3'.")
    ap.add_argument("--device", default=None, help="single-GPU alias (int or cuda:N); overrides --devices.")
    ap.add_argument("--threshold", type=float, default=None,
                    help="fake-decision threshold (default: checkpoint threshold@real95, else 0.5)")
    ...
    worker_devs = [d for d in devices for _ in range(wpg)]      # e.g. [2,2,3,3] for wpg=2
    ...
    resume_files = sorted(glob.glob(os.path.join(args.out_dir, "results_selop.shard*.txt"))) \
    ...
    out_file = merge_shard_files(args.out_dir, "selop", devices, [tag, col])
